...
void write_inode_table(int fd) {
	...
	char hello_symlink_array[]="hello-world";
	memcpy(hello_symlink_inode.i_block,hello_symlink_array,11);
...
struct ext2_dir_entry lost_and_found = {0};
	...
	bytes_remaining -= lost_and_found.rec_len;
 	...
 	struct ext2_dir_entry hello_symlink = {0};
	dir_entry_set(hello_symlink, HELLO_INO, "hello");
	dir_entry_write(hello_symlink, fd);
	bytes_remaining -= hello_symlink.rec_len;
...
void write_hello_world_file_block(int fd) {
	...
	ssize_t size = sizeof(hello_world_text);
	if (write(fd, &hello_world_text, size) != size) {
		errno_exit("write");
	}
